...
CNNtable = initialUniformCNNfilters(lr); savedCNNtable = initialUniformCNNfilters(lr);
...
for i = 1:Nepochs
  for j = 1:(floor(size(x_train, 1) / batch_size) - 1)
    ...
    CNNtable = uniformCNNfilters(lr, savedCNNtable);
    CNNtable = layerByLayer(count, CNNtable, savedCNNtable);
...
save(fullfile(mainFolder, "Accelerated-Blind-CNN-Descent-ABCD-", "outputs", "five.mat"), "saveStruct");